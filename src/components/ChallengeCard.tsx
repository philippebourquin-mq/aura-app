...
export function ChallengeCard({ challenge, status, onMarkDone }: Props) {
  ...
  return (
    <div className="w-full max-w-xs overflow-hidden rounded-card border border-black/10 bg-cream shadow-sm">
      <div
      ...
      <div className="px-5 py-5">
        <p className="font-rounded text-sm leading-relaxed text-black/80">
          {challenge.description}
        </p>

        <div className="mt-6 flex items-center justify-between">
          <span className="font-rounded text-xs tracking-[0.3em] text-black/20">AURA</span>
          <span
        ...
        {status !== 'todo' && (
          <div className="mt-3 flex items-center gap-1 text-xs font-semibold text-black/60">
            {badge.icon}
            {badge.label}
          </div>
        )}
        ...
        {onMarkDone && status === 'todo' && (
          <button
            onClick={onMarkDone}
            className="mt-4 w-full rounded-full bg-black py-2 text-sm font-semibold text-cream transition hover:bg-black/80"
          >
            Je l'ai fait
          </button>
        )}
